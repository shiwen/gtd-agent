...
export default function ScheduledPage() {
  const { loadTasks, getScheduledTasks, updateTask, deleteTask } = useTaskStore();
  ...
  let scheduledTasks = getScheduledTasks();

  // Sort by scheduled date
  scheduledTasks = [...scheduledTasks].sort((a, b) => {
        ...
        {scheduledTasks.length === 0 ? (
          <div className="text-center py-12 text-muted-foreground">
            <p className="text-lg mb-2">
              {searchQuery ? '没有找到匹配的任务' : '暂无已安排的任务'}
            </p>
            <p className="text-sm">
              {searchQuery ? '尝试其他搜索词' : '为任务设置日期后，它们会出现在这里'}
            </p>
          </div>
        ) : (
          <div>
